...
// 但是如果要播放的媒体是视讯档案(例如:MV)的话，最好将这个设定值改为 false 。
// true = 自动连续播放
...
showTLab();
disp1.innerHTML = "音视频播放系统，基于WindowsMediaPlayer设计 － 点击下面的播放按钮将顺序自动播放，也可以点击下面的目录按钮选择播放！";
if(blnStatusBar){ window.status=('请选择右边的按钮');}
...
Exobud.close();
disp1.innerHTML = "网站媒体播放程序就绪";
if(blnStatusBar){ window.status=('网站媒体播放程序就绪');return true;}
}
...
if(f==8){capText.innerHTML="<p>CopyRight&nbsp;&copy;&nbsp;2006 - 2008&nbsp;";}
if(f==13){
...
// 只可以在 vmin, vmax, vdep 设为0至100之间的整数数值，vmin 和 vdep 数值不可以大过 vmax。
var vmax = 100;
var vmin = 0;
var vdep = 10;
...
function waitMsg(){
capText.innerHTML="<p>CopyRight&nbsp;&copy;&nbsp;2004 - 2005&nbsp;";
if(intExobudStat==1){disp1.innerHTML = "无法播放 － 播放清单上没有设定任何曲目。";}
if(intExobudStat==2){disp1.innerHTML = "无法播放 － 没有选取播放清单上任何一首曲目。";}
if(blnStatusBar){
if(intExobudStat==1){ window.status=('无法播放 － 播放清单上没有设定任何曲目。'); return true;}
if(intExobudStat==2){ window.status=('无法播放 － 没有选取播放清单上任何一首曲目。'); return true;}
}
}
// openPlist() 函式: 以弹出窗口显示播放清单内容
function openPlist(){
window.open("exobudpl.htm","mplist","left=30,width=271,height=373,scrollbars=no,resizable=no,copyhistory=no");
}
...
function vizExobud(){
// 使用范例: <span onClick="vizExobud()" style="cursor:hand" title="到访 baidu 的网站">
window.open("http://www.baidu.com","vizExobud");
}
//-->