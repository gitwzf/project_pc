...
//db.class.php
class db_class {
	var $dbserver = "localhost"; 		//数据库服务器,需要修改
	var $dbuser = "root";        		//数据库登录帐号,需要修改
	var $dbpassword = "";        		//数据库密码,需要修改
	var $database = "phpbook_ch16";     //数据库名,需要修改

	//-----------------------------------------------------------------------------------
	// 以下部分不用修改!
	//-----------------------------------------------------------------------------------
	var $link_id  = 0;             //数据库连接ID号
	var $query_id = 0;
	var $record   = array();       //记录
	...
	function connect() {
		...
		if ( 0 == $this->link_id ) {
			...
			//使用mysql_pconnect比较节省资源,不需要用mysql_close关闭连接
			if (!$this->link_id) {
	...
	function halt($msg) {
		...
		if ($this->reporterror==1) {
			...
			echo "
				...
				<p>
				如果问题持续出现，请告诉
				<a href=mailto:cheng@sina.net?subject=".urlencode('数据库问题：'.$msg).">cheng</a>.
				或者访问 <a href=\"http://forum.com.net\">cheng 论坛</a> 寻求解决.
				</p>
