			...
			<tr class="control-group">
				<td class="rule-manage">
					<span class="rule-type pull-right">
										...
										<div>
						<c:if test="${txt.type!='-1'}"><c:if test="${txt.identity!='welcome'}"><a href="le111Serv?name=${txt.name}&act=setwelcome" onclick="" style="color:#FF3300">设为欢迎信息</a></c:if><c:if test="${txt.identity=='welcome'}"><a href="le111Serv?name=${txt.name}&act=delwelcome" onclick="" style="color:#FF3300">取消欢迎</a></c:if>
						                               <c:if test="${txt.identity!='default'}"><a href="le111Serv?name=${txt.name}&act=setdefault" onclick="">设为默认回复</a></c:if><c:if test="${txt.identity=='default'}"><a href="le111Serv?name=${txt.name}&act=deldefault" onclick="">取消默认</a></c:if></c:if>
											</div>
				</td>
			</tr>
		</table>
